...
void CGameEdu01::OnInit()
{
	...
	// 주인공 충돌 정점 설정 ( 최대 최소 정점 값 <= 모델 좌표 ), 최대점, 최소점을 따온다.	
	...
	// 주전자 주인공 캐릭터는 초반 설정에서 -90도 회전을 했으므로 충돌 정점들도 똑같이 회전을 해주어야 한다.
	D3DXVECTOR3 vMin1, vMax1;
	float fTemp;
	D3DXVec3TransformCoord(&vMin1, &m_sPlayer.vMin, &m_sPlayer.matRotationY); // 최소점 -90도 회전
	D3DXVec3TransformCoord(&vMax1, &m_sPlayer.vMax, &m_sPlayer.matRotationY); // 최대점 -90도 회전
	...
	if (vMin1.y > vMax1.y) // 모델좌표에서 회전이 있으므로 최대점과 최소점을 다시 구해야 한다.
	{
		fTemp = vMin1.y;
		vMin1.y = vMax1.y;
		vMax1.y = fTemp;
	...
	if (vMin1.z > vMax1.z) // 모델좌표에서 회전이 있으므로 최대점과 최소점을 다시 구해야 한다.
	{
		fTemp = vMin1.z;
		vMin1.z = vMax1.z;
		vMax1.z = fTemp;
	...
	// 실질적인 최소점, 최대점이 나왔으니 다시 옮겨준다. 이렇게 최대점, 최소점이 회전을 통해 바뀌게 되면 위와 같이 비교를 통해 다시 값을 바로 잡아주어야 한다.
...
void CGameEdu01::OnUpdate()
{
	...
	D3DXMATRIX matWorld; // 각종 변환을 위한 버퍼 행렬
	...
	// 주인공 총알, 주인공 몸통에 관한 충돌 최소점, 최대점
	D3DXVECTOR3 vMin1, vMax1;
	// 적 총알, 적 몸통에 관한 충돌 최소점, 최대점
	D3DXVECTOR3	vMin2, vMax2;

	// 이미 Oninit에서 충돌점 회전을 해주었기에 이동 행렬, 스케일 행렬만 충돌점에 영향을 준다.
	matWorld = m_sPlayer.matScale * m_sPlayer.matTranslation;
	D3DXVec3TransformCoord(&vMin1, &m_sPlayer.vMin, &matWorld); // 캐릭터 충돌 최소점 따오기
	D3DXVec3TransformCoord(&vMax1, &m_sPlayer.vMax, &matWorld); // 캐릭터 충돌 최대점 따오기
	...
	for (i = 0; i < m_nEnemyIndex; i++)
	{
		if (m_Enemy[i].nLife > 0)
		{
			...
			D3DXVec3TransformCoord(&vMin2, &m_EnemyProperty.vMin, &matWorld); // 총알 충돌 최소점 따오기
			D3DXVec3TransformCoord(&vMax2, &m_EnemyProperty.vMax, &matWorld); // 총알 충돌 최대점 따오기

			if (CheckCubeIntersection(&vMin1, &vMax1, &vMin2, &vMax2)) // 충돌을 했다면 적 체력 없애고 캐릭터 체력을 깎는다.
			{
				m_Enemy[i].nLife = 0;
				m_sPlayer.nLife--;
			}
	...
	for (i = 0; i < 100; i++)
	{
		if (m_EnemyBullet[i].nLife > 0)
		{
			...
			if (CheckCubeIntersection(&vMin1, &vMax1, &vMin2, &vMax2)) // 충돌 했다면
			{
				m_EnemyBullet[i].nLife = 0; // 총알 제거
				m_sPlayer.nLife--; // 캐릭터 체력 깎기
			}
		}
	}
		...
		if (m_sPlayerBullet[i].nLife > 0)
		{
			...
			for (j = 0; j < m_nEnemyIndex; j++)
			{
				if (m_Enemy[j].nLife > 0)
				{
					...
					D3DXVec3TransformCoord(&vMin2, &m_EnemyProperty.vMin, &matWorld); // 최소점
					D3DXVec3TransformCoord(&vMax2, &m_EnemyProperty.vMax, &matWorld); // 최대점

					if (CheckCubeIntersection(&vMin1, &vMax1, &vMin2, &vMax2)) // 충돌 했는가?
					{
						m_Enemy[j].nLife = 0;
						m_sPlayerBullet[i].nLife = 0;
						break; // 여러개의 적이 겹쳐나오는 일이 없기에 충돌한 적이 있다면 더 확인하지 않고 빠져나온다.
					}
				}
			}

			// Note: 주인공 총알과 적 총알 충돌 체크
			// 플레이어가 발사한 총알과 어떤 적의 총알이 맞았는지 판단
			if (m_sPlayerBullet[i].nLife > 0) // 위에서 적의 몸통과 플레이어의 총알이 맞았을 수도 있기에 조건이 추가된다.
			{
				for (j = 0; j < 100; j++)
				{
					if (m_EnemyBullet[j].nLife > 0)
					{
						...
						matWorld = m_EnemyBulletProperty.matScale * m_EnemyBullet[j].matTranslation;
						D3DXVec3TransformCoord(&vMin2, &m_EnemyBulletProperty.vMin, &matWorld);
						D3DXVec3TransformCoord(&vMax2, &m_EnemyBulletProperty.vMax, &matWorld);
						...
						if (CheckCubeIntersection(&vMin1, &vMax1, &vMin2, &vMax2))
						{
							...
							m_sPlayerBullet[i].nLife = 0;
							break; // 여러개의 총알이 겹쳐나오는 일이 없기에 충돌한 총알이 있다면 더 확인하지 않고 빠져나온다. 
						}
					}
				}
			}
