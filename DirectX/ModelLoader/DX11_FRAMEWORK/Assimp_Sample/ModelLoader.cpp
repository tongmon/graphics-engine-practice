	...
	for (UINT i = 0; i < mat->GetTextureCount(type); i++) {
		...
		for (int j = str.length - 1; j >= 0; j--) {
			if (str.data[j] == '/' || str.data[j] == '\\')
				break;
			buf.push_back(str.data[j]);
		}
		std::reverse(buf.begin(), buf.end());
		// buf = "textures/" + buf; // 폴더 이름 지정할 거면 이렇게
		memset(str.data, 0, sizeof(str.data));
		for (int j = 0; j < buf.size(); j++)
			str.data[j] = buf[j];
		str.data[buf.size()] = '\0';
		str.length = ai_uint(buf.size() - 1);
		...
		if (!skip) {   // If texture hasn't been loaded already, load it
			...
			if (textype == "embedded compressed texture") {
				...
				if (FAILED(hr))
					MessageBox(hwnd_, TEXT("Texture couldn't be loaded"), TEXT("Error!"), MB_ICONERROR | MB_OK);
			}
...
ID3D11ShaderResourceView * ModelLoader::getTextureFromModel(const aiScene * scene, int textureindex) {
	...
	if (FAILED(hr))
		MessageBox(hwnd_, TEXT("Texture couldn't be created from memory!"), TEXT("Error!"), MB_ICONERROR | MB_OK);
