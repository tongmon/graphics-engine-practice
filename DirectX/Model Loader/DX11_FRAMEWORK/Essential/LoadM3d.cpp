...
// 정점마다 어떤 뼈에 영향을 받는지에 대한 가중치와 그에 해당 하는 뼈가 어떤 것인지가 모두 담겨있다.
// 이를 이용하면 뼈에 부드럽에 연결된 표피 텍스쳐를 볼 수가 있다.
...
// 뼈대가 뿌리 뼈에서 얼마나 떨어져있는지 저장해놓은 행렬 읽기
...
// 애니메이션 클립 읽고 애니메이션 맵에 추가하는 함수
// 각 애니메이션에는 각 뼈들의 키 프레임들이 들어있다.
// 애니메이션 --> 여려 개의 뼈들
// 한 개의 뼈 --> 여러 개의 각 시간 마다의 위치 (키 프레임)
								   ...
								   std::map<std::string, AnimationClip>& animations)
{
    ...
    for(UINT clipIndex = 0; clipIndex < numAnimationClips; ++clipIndex)
    {
        ...
        animations[clipName] = clip; // 애니메이션 맵에 추가
    }
}
