...
{
	D3DXMATRIX matRotate, matRotX, matRotY, matRotZ, matResult; // 회전 조정하기 위한 행렬
	float fYaw, fPitch, fRoll;
	fYaw = D3DXToRadian(45);
	fPitch = D3DXToRadian(45);
	fRoll = D3DXToRadian(90);
	...
	D3DXMatrixRotationX(&matRotX, fPitch);
	D3DXMatrixRotationZ(&matRotZ, fRoll);
	matResult = matRotZ * matRotX * matRotY;

	// 한번에 각도 3개 회전, 물론 함수 삽입은 Y -> X -> Z 순
	// 적용 순서는 Z -> X -> Y 순
	D3DXMatrixRotationYawPitchRoll(&matRotate, fYaw, fPitch, fRoll);

	m_pd3dDevices->SetTransform(D3DTS_WORLD, &matResult); // 따로 따로 각도 회전해준 행렬
	//m_pd3dDevices->SetTransform(D3DTS_WORLD, &matRotate); // YawPitchRoll로 한번에 각도 회전해준 행렬
