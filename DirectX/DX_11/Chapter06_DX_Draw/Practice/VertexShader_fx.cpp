// BoxDemo 기반
// 정점 쉐이더에서 무슨 역할을 하는지 알기 좋은 연습
// 정점 쉐이더에 사각형을 시간에 따라 슬라임 처럼 만들어주는 변형을 추가한다.
// 변경점은 BuildFX에 현재 Cpp 파일과 fx 파일에 시간이라는 변수를 중계해야 하는데
// 이를 가능케 하는 ID3DX11EffectScalarVariable* 변수 하나를 선언하고 중계시킨다.
// DrawScene 함수에서 현재 시각을 획득해서 fx 파일에 있는 gTime에 넣어주는 부분이 추가된다.
// 수정되는 fx 파일은 코드 맨 밑부분에 추가했으니 까먹겠다 싶으면 보자.
// 그리고 fx 파일 다 변경하고 "왜 적용이 안되지?" 라고 생각이 들면....
// Debug 폴더의 fx 파일도 변경했는지 확인해보자.... 괜히 삽질하지 말고
...
private:
	...
	ID3DX11EffectScalarVariable* mfxTime; // 쉐이더(fx)와 gTime 변수를 상호작용시키기 위한 변수
...
BoxApp::BoxApp(HINSTANCE hInstance)
	...
	mfxWorldViewProj(0), mInputLayout(0), mfxTime(0),
...
bool BoxApp::Init() // 초기화
{
	...
	wireframeDesc.FillMode = D3D11_FILL_SOLID; // 솔리드 모드
...
void BoxApp::DrawScene()
{
	...
	// 시간에 따라 왜곡 진행, 정점 쉐이더(fx)와 연계
	// 이러면 mTimer.TotalTime()의 값이 fx 파일의 gTime으로 들어가면서 
	// 각 패스마다 gTime에 따라 변경되는 최종 좌표 값이 정점에 적용된다.
...
void BoxApp::BuildFX()
{
	...
	mTech = mFX->GetTechniqueByName("ColorTech"); // fx 파일을 보면 ColorTech라는 이름의 테크닉 구조체가 있다. 이를 가져오는 것
	mfxWorldViewProj = mFX->GetVariableByName("gWorldViewProj")->AsMatrix(); // fx 파일에서 gWorldViewProj 변수를 행렬 형태로 획득함
	
	// 현재 시각을 얻은 C++ 파일에서 fx와 상호작용하기위해
	// 알맞은 값을 대응시켜줘야 한다.
	// 여기서 알맞은 값은 mTimer.TotalTime() <--> mfxTime <--> gTime
	// mfxTime이 중계기 역할을 한다.
	// mTimer.TotalTime()은 현재 시각 획득 함수, GameTimer에 소속
...
cbuffer cbPerObject
{
	...
	float gTime; // gTime 시간 변수 추가, 추후에 Cpp에서 획득한 값과 연계
};
