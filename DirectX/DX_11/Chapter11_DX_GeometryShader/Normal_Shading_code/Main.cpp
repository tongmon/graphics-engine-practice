...
// 나무 빌보드 예제
// 나무와 같이 특정한 형태인데 많이 심어야되는 물체들은
// 실제로 3D로 다 심어서 출력하면 리소스를 많이 잡아먹기에
// 카메라에 가까운 것만 3D로 풀로 출력하고 먼 것은 이미지만 출력한다.
// 이미지도 카메라의 시점과 관계없이 그냥 박혀있으면 이미지인 것이 티나 나기에
// 카메라가 바라보는 방향을 따라서 계속 움직여주어야 플레이어가
// 알아차리기 힘들다.
// 이 예제에서는 나무 중심 점 하나를 잡아주면 기하쉐이더가 이를 사각형으로 확장하고
// 그 사각형이 카메라 위치에 따라 움직이게까지 해준다.
...
void TreeBillboardApp::DrawScene()
{
	...
	ID3DX11EffectTechnique* DebugTech = Effects::BasicFX->Light3DebugTech;
	...
	for (UINT p = 0; p < techDesc.Passes; ++p)
	{
		...
		//md3dImmediateContext->OMSetBlendState(RenderStates::AlphaToCoverageBS, blendFactor, 0xffffffff);
		md3dImmediateContext->RSSetState(RenderStates::NoCullRS);
		DebugTech->GetPassByIndex(p)->Apply(0, md3dImmediateContext);
		md3dImmediateContext->DrawIndexed(36, 0, 0);
