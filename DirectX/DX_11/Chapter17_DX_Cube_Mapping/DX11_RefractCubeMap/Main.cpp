// 굴절 처리
// 주요 수정 내용으로는 Basic.fx에서 refract 함수를 사옹한 부분이다.
// 반사효과와 별 다를 것 없이 반사는 반사되는 벡터를 구하고 그 벡터가
// 가리키는 위치의 텍셀이 해당 도형에 입혀졌다면 굴절은 굴절되는 벡터가
// 해당 도형에 입혀지는 것이다.
// refract 함수가 이 굴절 벡터를 알아서 구해준다.
	...
	mSkullIndexCount(0), mLightCount(3)
{
	...
	mSphereMat.Diffuse = XMFLOAT4(0.4f, 0.4f, 0.4f, 1.0f);
