...
// 여기서는 이동 끝점을 수평이동으로만 측정한다.
// 즉 시점벡터의 y 원소를 0으로 해둔채로 x,z를 획득하고
// 그 획득한 좌표를 통해서 이동 끝점의 x,y,z를 구한다.
...
void TerrainApp::UpdateScene(float dt)
{
	...
	XMFLOAT3 Look = mCam.GetLook();
	float LookLen = sqrtf(powf(Look.x, 2) + powf(Look.z, 2));
	Look = { Look.x / LookLen, 0, Look.z / LookLen };

	if (GetAsyncKeyState('W') & 0x8000) {
		if (mWalkCamMode)
			mCam.Move(Look, 10.0f * dt);
		else
			mCam.Walk(10.0f * dt);
	}
