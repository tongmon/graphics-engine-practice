// 텍스쳐 회전 변환시에 법선맵을 어떻게 적용시키는지
// 해결하는 예제 문제에 대한 답이다.
// 별건 없고 회전이 된 만큼 법선 T를 똑같이 회전해 주면 된다.
...
// 텍스쳐 회전 변환을 고려한 법선 맵 세계 공간으로 위치 시키는 함수
// 텍스쳐가 회전하면 법선 맵도 그에 맞게 회전을 해야한다.
// 이를 적용하기 위해 세계 공간에서 정의된 상태로 구해지는 T를 다시 접공간(텍스쳐)으로 돌려놓고
// 이 상태에서 텍스쳐 변환 행렬을 적용하여 T를 옮기고 다시 그 T를 세계 공간으로 옮겨 놓는 것이다.
float3 NormalSampleToWorldSpaceWithMatrix(float3 normalMapSample, float3 unitNormalW, float3 tangentW, float4x4 TexTrans)
{
	...
	// TBN은 세계공간 기준에서의 3개의 축 벡터이다.
	// 즉 TBN은 현재 세계 공간 좌표계 기준으로 서술이 되어 있는 상태이다.
    float3x3 TBN = float3x3(T, B, N); // 접공간 --> 세계 공간
    float3x3 revTBN = transpose(TBN); // 세계 공간 --> 접공간
    T = mul(T, revTBN); // T를 접공간으로...
    T = mul(float4(T, 0.0f), TexTrans).xyz; // 회전 행렬 적용
    T = mul(T, TBN); // T를 세계 공간으로...
	
    T = normalize(T - dot(T, N) * N); // tangentW, unitNormalW 사이의 각을 90도로 만들기 위한 변형
    B = cross(N, T);
    TBN = float3x3(T, B, N); // 새롭게 구축된 TBN
