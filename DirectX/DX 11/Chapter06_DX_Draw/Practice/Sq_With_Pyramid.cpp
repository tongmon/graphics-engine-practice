// BoxDemo 기반
// 정점 버퍼와 인덱스 버퍼 연습
// 여러 도형을 한꺼번에 몰아 넣는 연습을 했다.
// Shapes 예제를 잘 봤다면 쉽게 접근이 가능하다.
// 변한 점은 BuildGeometryBuffers 함수에서 정점, 인덱스 선언
// 그리고 DrawScene 함수에서 이동변환을 가해준 것 정도가 있다. 
...
void BoxApp::DrawScene()
{
	...
	{		
		// 박스 그리기
		XMMATRIX S = XMMatrixScaling(0.7f, 0.7f, 0.7f), T = XMMatrixTranslation(-0.8f, 0.0f, 0.0f);
		worldViewProj = S * T * ViewProj;
		mfxWorldViewProj->SetMatrix(reinterpret_cast<float*>(&worldViewProj));
		mTech->GetPassByIndex(p)->Apply(0, md3dImmediateContext);
		// 큐브 인덱스 36개, 인덱스 시작부(처음에 큐브부터 넣었으니 0), 정점 시작부(큐브부터 넣었으니 0)
		md3dImmediateContext->DrawIndexed(36, 0, 0);

		// 피라미드 그리기
		T = XMMatrixTranslation(+0.8f, 0.0f, 0.0f);
		worldViewProj = S * T * ViewProj;
		mfxWorldViewProj->SetMatrix(reinterpret_cast<float*>(&worldViewProj));
		mTech->GetPassByIndex(p)->Apply(0, md3dImmediateContext);
		// 인덱스 개수 18개, 인덱스 시작 위치(큐브 크기 후에 시작), 정점 시작부(큐브 정점 8개니까 그 후에)
...
void BoxApp::BuildGeometryBuffers() // 정점 정의 함수
{
	...
	std::vector<Vertex> TotalV;
	for (size_t i = 0; i < sizeof(CubeVert) / sizeof(Vertex); i++)
		TotalV.push_back(CubeVert[i]);
	for (size_t i = 0; i < sizeof(Pyramid) / sizeof(Vertex); i++)
		TotalV.push_back(Pyramid[i]);

	D3D11_BUFFER_DESC vbd;
	// 보통은 D3D11_USAGE_DEFAULT를 해서 GPU가 자원을 읽고 쓰게 하지만 
	// 여기선 최적화를 위해 버퍼 생성 후에 전혀 변경이 없는 D3D11_USAGE_IMMUTABLE를 사용
	vbd.Usage = D3D11_USAGE_IMMUTABLE;
	vbd.ByteWidth = sizeof(Vertex) * TotalV.size(); // 생성할 정점 버퍼의 크기
	...
	std::vector<UINT> TotalInd;
	for (size_t i = 0; i < sizeof(CubeInd) / sizeof(UINT); i++)
		TotalInd.push_back(CubeInd[i]);
	for (size_t i = 0; i < sizeof(PyrInd) / sizeof(UINT); i++)
		TotalInd.push_back(PyrInd[i]);

	// 인덱스 버퍼 특성 지정, 방식은 정점 버퍼와 비슷
	D3D11_BUFFER_DESC ibd;
	ibd.Usage = D3D11_USAGE_IMMUTABLE;
	ibd.ByteWidth = sizeof(UINT) * TotalInd.size(); // 36개의 인덱스
	ibd.BindFlags = D3D11_BIND_INDEX_BUFFER; // 인덱스 버퍼는 이렇게 지정
	ibd.CPUAccessFlags = 0;
	ibd.MiscFlags = 0;
	ibd.StructureByteStride = 0;
	D3D11_SUBRESOURCE_DATA iinitData; // 색인 버퍼 초기화할 자료를 지정
	iinitData.pSysMem = &TotalInd[0];
