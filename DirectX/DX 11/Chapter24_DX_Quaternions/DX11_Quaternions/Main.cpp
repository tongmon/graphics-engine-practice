...
// 회전시에 회전 축이 혼동되는 현상인 짐벌락을 방지해주는 사원수(quaternion)를 이용한 회전 예제다.
// 애니메이션도 약간 다루고 있다.
// 사원수는 짐벌락을 완전히는 아니고 많이 해결해주고 행렬 회전은 16x16 연산인데 사원수를 사용하면 4x4에
// 끝나는 효과가 있어서 좋다고 한다. (근데 그래픽스에서는 사실상 행렬로 묶는게 더 가벼워진다.)
// 사원수는 일단 (nsin, cos) 여기서 n은 단위 벡터이고 회전축을 나타내고 각 sin, cos에 들어가는 세타값이
// 회전 각을 나타낸다. 숫자가 4개로 구성된 복소수 형태라 이름이 사원수이다.
// 사원수는 보간이 용이해서 애니메이션에 사용하기 매우 적합하다.
	...
	mSkullIndexCount(0), mAnimTimePos(0.0f)
{
	...
	// 애니메이션은 시각별로 키프레임을 몇 개 주고 여기서 각 프레임 사이 특성들은
	// 보간을 통해 획득한다.
...
void QuatApp::UpdateScene(float dt)
{
	...
	// 시간 증가시키다가 애니메이션 끝 시간에 다다르면 다시 초기화
	mAnimTimePos += dt;
	...
	// 보간이 진행되는 부분, 지금 순간의 두개골 세계 행렬을 구함.
