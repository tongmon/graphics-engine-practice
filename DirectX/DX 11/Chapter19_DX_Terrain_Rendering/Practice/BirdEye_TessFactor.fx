// 테셀레이션이 잘 이루어지고 있는지 확인하려면
// 위에서 지형을 내려다봐야할 상황이 생긴다.
// 이러한 상황에서는 y좌표는 거리 측정에서 빼야한다.
// 이를 CalcTessFactor 함수에 반영했다.
...
{
    // xz 평면의 최대 크기(조감도 시점에서 세부수준들을 살펴보기에 유용하다.)
	// 테셀레이션 감도를 조절하는데 y와의 거리는 무시되기에
	// 새처럼 높이 날아서 밑을 보는 시점에서 이동에 따라 테셀레이션이 잘 진행되는지
	// 확인하는 곳에 용이하다.
