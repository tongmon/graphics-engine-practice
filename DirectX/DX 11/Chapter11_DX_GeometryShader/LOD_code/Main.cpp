...
private:
	...
	void BuildSpereGeometryBuffers();
...
private:
	...
	ID3D11Buffer* mSpereVB;
	ID3D11Buffer* mSpereIB;
...
TreeBillboardApp::TreeBillboardApp(HINSTANCE hInstance)
	...
	mGrassMapSRV(0), mWavesMapSRV(0), mBoxMapSRV(0), mAlphaToCoverageOn(true), mSpereVB(0), mSpereIB(0),
	...
	mTheta(1.3f * MathHelper::Pi), mPhi(0.4f * MathHelper::Pi), mRadius(80.0f)
{
	...
	XMMATRIX SpereScale = XMMatrixScaling(15.0f, 15.0f, 15.0f);
	XMMATRIX SpereOffset = XMMatrixTranslation(10.0f, 20.0f, 10.0f);
	XMStoreFloat4x4(&mSpereWorld, SpereScale * SpereOffset);
	...
	mSpereMat.Ambient = XMFLOAT4(0.5f, 0.5f, 0.5f, 1.0f);
	mSpereMat.Diffuse = XMFLOAT4(1.0f, 1.0f, 1.0f, 1.0f);
	mSpereMat.Specular = XMFLOAT4(0.4f, 0.4f, 0.4f, 16.0f);
...
void TreeBillboardApp::DrawScene()
{
	...
	ID3DX11EffectTechnique* spereTech = NULL;
	...
	case RenderOptions::Lighting:
		...
		spereTech = Effects::BasicFX->Light3SphereTech;
	...
	case RenderOptions::TexturesAndFog:
		...
		spereTech = Effects::BasicFX->Light3SphereFogTech;
	...
	spereTech->GetDesc(&techDesc);
	for (UINT p = 0; p < techDesc.Passes; ++p)
	{
		md3dImmediateContext->IASetVertexBuffers(0, 1, &mSpereVB, &stride, &offset);
		md3dImmediateContext->IASetIndexBuffer(mSpereIB, DXGI_FORMAT_R32_UINT, 0);

		// Set per object constants.
		XMMATRIX world = XMLoadFloat4x4(&mSpereWorld);
		...
		Effects::BasicFX->SetMaterial(mSpereMat);

		//md3dImmediateContext->OMSetBlendState(RenderStates::AlphaToCoverageBS, blendFactor, 0xffffffff);
		md3dImmediateContext->RSSetState(RenderStates::NoCullRS);
		spereTech->GetPassByIndex(p)->Apply(0, md3dImmediateContext);
		md3dImmediateContext->DrawIndexed(mSpereIndCnt, 0, 0);
...
void TreeBillboardApp::BuildSpereGeometryBuffers()
{
	GeometryGenerator::MeshData spere;

	GeometryGenerator geoGen;
	geoGen.CreateGeosphere(1, 0, spere);
	...
	std::vector<Vertex::Basic32> vertices(spere.Vertices.size());

	for (UINT i = 0; i < spere.Vertices.size(); ++i)
	{
		vertices[i].Pos = spere.Vertices[i].Position;
		vertices[i].Normal = spere.Vertices[i].Normal;
		vertices[i].Tex = spere.Vertices[i].TexC;
	}

	D3D11_BUFFER_DESC vbd;
	vbd.Usage = D3D11_USAGE_IMMUTABLE;
	vbd.ByteWidth = sizeof(Vertex::Basic32) * spere.Vertices.size();
	vbd.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	vbd.CPUAccessFlags = 0;
	vbd.MiscFlags = 0;
	D3D11_SUBRESOURCE_DATA vinitData;
	vinitData.pSysMem = &vertices[0];
	HR(md3dDevice->CreateBuffer(&vbd, &vinitData, &mSpereVB));

	mSpereIndCnt = spere.Indices.size();
	...
	D3D11_BUFFER_DESC ibd;
	ibd.Usage = D3D11_USAGE_IMMUTABLE;
	ibd.ByteWidth = sizeof(UINT) * spere.Indices.size();
	ibd.BindFlags = D3D11_BIND_INDEX_BUFFER;
	ibd.CPUAccessFlags = 0;
	ibd.MiscFlags = 0;
	D3D11_SUBRESOURCE_DATA iinitData;
	iinitData.pSysMem = &spere.Indices[0];
	HR(md3dDevice->CreateBuffer(&ibd, &iinitData, &mSpereIB));
}
