...
void InstancingAndCullingApp::UpdateScene(float dt)
{
	...
	if (mFrustumCullingEnabled)
	{
		...
		for (UINT i = 0; i < mInstancedData.size(); ++i)
		{
			...
			if (XNA::IntersectSphereFrustum(&mSkullSphere, &localspaceFrustum) != 0)
			{
...
void InstancingAndCullingApp::BuildSkullGeometryBuffers()
{
	...
	// 충돌 구 판정은 일단 중심 구하는 과정은 AABB와 동일하고 구의 반지름은 그 중심에서 가장 먼
	// 정점과의 거리가 된다.
	// XNA 라이브러리에 이를 수행해주는 함수가 구비되어 있어 사용했다.
	...
	XNA::ComputeBoundingSphereFromPoints(&mSkullSphere, vcount, &vertices[0].Pos, sizeof(Vertex::Basic32));
