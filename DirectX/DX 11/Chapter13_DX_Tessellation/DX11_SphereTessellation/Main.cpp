...
BasicTessellation::BasicTessellation(HINSTANCE hInstance)
	: D3DApp(hInstance), mSpereIB(0), mSpereVB(0),
...
void BasicTessellation::DrawScene()
{
	...
	// 제어점 3개로 정이십면체의 삼각형마다 테셀레이션 해준다.
	...
	for (UINT p = 0; p < techDesc.Passes; ++p)
	{
		...
		XMMATRIX world = XMMatrixScaling(20, 20, 20);
...
void BasicTessellation::BuildQuadPatchBuffer()
{
	...
	std::vector<Vertex::Pos> vertices(spere.Vertices.size());

	for (UINT i = 0; i < spere.Vertices.size(); ++i)
	{
		vertices[i].x = spere.Vertices[i].Position.x;
		vertices[i].y = spere.Vertices[i].Position.y;
		vertices[i].z = spere.Vertices[i].Position.z;
	}

	D3D11_BUFFER_DESC vbd;
	vbd.Usage = D3D11_USAGE_IMMUTABLE;
	vbd.ByteWidth = sizeof(Vertex::Pos) * spere.Vertices.size();
