...
void CameraApp::OnResize()
{
	...
	mCam.SetLens(0.25f * MathHelper::Pi, AspectRatio(), 1.0f, 1000.0f); // 창이 바뀌니 종횡비를 바꾼다.
}
...
void CameraApp::UpdateScene(float dt)
{
	...
	if (GetAsyncKeyState('Q') & 0x8000)
		mCam.Roll(XMConvertToRadians(0.1));
...
{
	if ((btnState & MK_LBUTTON) != 0) // 좌클릭 시에
	{
		// 픽셀 하나당 1/4 도에 해당한다.
