...
void PickingApp::BuildMeshGeometryBuffers()
{
	...
	XNA::ComputeBoundingSphereFromPoints(&mMeshSphere, vcount, &mMeshVertices[0].Pos, sizeof(Vertex::Basic32));
...
void PickingApp::Pick(int sx, int sy)
{
	...
	mPickedTriangle = -1;
	float tmin = 0.0f;
	if (XNA::IntersectRaySphere(rayOrigin, rayDir, &mMeshSphere, &tmin)) // 반직선과 Sphere가 충돌했다면
	{
