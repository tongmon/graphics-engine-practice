...
// m3d 파일을 적재하는 방식을 알려주는 예제이다.
// m3d 파일은 별게 아니라 그냥 사용자가 정의한 텍스쳐 파일이다. 근데 여기에
// 정점, 텍스쳐 파일명, 삼각형 개수... 등이 다 적혀있다.
// 이 파일을 그냥 입맛대로 읽어서 캡슐화 해놓은 클래스들이 meshgeomery.h, basicmodel.h 이다.
// meshgeomery 얘는 해당 모델의 정점, 인덱스를 통째로 담고 있고 그리기를 수행할 때는 서브셋 범위를
// 지정해주면 그 범위에 해당하는 녀석을 그린다.
// basicmodel은 모델 각 부분 (ex. 차 바퀴, 유리창)에 대한 텍스쳐를 담고 있다. (추가적으로 CPU 계산을 위한 정점도 담는다.)
