...
void PathFinderFast::InitPathFinder(Map* Map)
{
    if (Map == NULL) {
        return;
    }
...
void PathFinderFast::FindPathStop()
{
    mStop = true;
}

short PathFinderFast::GetGrid(int x, int y)
{
    if (x < 1 || x >= mMap->mWidth - 1 || y < 1 || y >= mMap->mHeight - 1)
        return 0;
    return mGrid->at(y).at(x);
}

std::vector<POINT>* PathFinderFast::FindPath(POINT start, POINT end, int characterWidth, int characterHeight, short maxCharacterJumpHeight, bool useLedges)
{
    while (!mOpen->empty())
        mOpen->pop();
    ...
    if (mFound)
    {
        ...
        while (fNode.x != fNodeTmp.PX || fNode.y != fNodeTmp.PY)
        {
            ...
            if ((mClose.empty())
                ...
                || (GetGrid(fNode.x, fNode.y - 1) == 0 && mMap->IsOneWayPlatform(fPrevNode.x, fPrevNode.y - 1))
